...
async def handle_findapi_callback(client, callback_query: CallbackQuery):
    try:
        ...
        # Search for matches (case insensitive and space-tolerant)
        matches = []
        clean_search = search_term.replace(" ", "")
        for app in apps:
            app_name = app.get('name', '').lower()
            app_api = app.get('api', '').lower()
            clean_name = app_name.replace(" ", "")
            if (search_term in app_name or clean_search in clean_name or search_term in app_api or clean_search in app_api):
                matches.append(app)
...
def find_api(keyword, data):
    ...
    matching_results = [
        ...
        if kw in item.get('name', '').lower() or clean_kw in item.get('name', '').lower().replace(" ", "") or kw in item.get('api', '').lower() or clean_kw in item.get('api', '').lower()
    ]
